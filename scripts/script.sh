...
KEY=$(cat /tmp/creds/files/letsencrypt/live/$YOUR_SUBDOMAIN.bahsoftwarefactory.com/privkey.pem | sed -e 's/^/              /')
CERT=$(cat /tmp/creds/files/letsencrypt/live/$YOUR_SUBDOMAIN.bahsoftwarefactory.com/fullchain.pem | sed -e 's/^/              /')

## Backup Certs to S3
aws s3 cp /tmp/creds/files/letsencrypt/ s3://bsf-dev-deployments/$YOUR_CLUSTER_VALUE/files/letsencrypt --recursive
...
LB_DNS_NAME=$(aws elb describe-load-balancers \
