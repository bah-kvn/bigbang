...
sops -d stg/secrets.enc.yaml \
  | yq '.stringData."values.yaml"' \
  | yq '.istio.vault.tls.cert' \
  > /tmp/istio_vault_tls_cert.cert

sops -d stg/secrets.enc.yaml \
  | yq '.stringData."values.yaml"' \
  | yq '.istio.gateways.public.tls.cert' \
  > /tmp/istio_gateways_tls_cert.cert

sops -d stg/secrets.enc.yaml \
  | yq '.stringData."values.yaml"' \
  | yq '.addons.vault.ingress.cert' \
  > /tmp/addons_vault_ingress_cert.cert
