...
helm get values bigbang -n bigbang | tee bigbang.yaml
echo
echo "from bigbang-values.yaml"
(
echo "  .Values.addons.vault.ingress.cert     = $( cat bigbang.yaml | yq '.addons.vault.ingress.cert' )"
echo "  .Values.addons.vault.ingress.key      = $( cat bigbang.yaml | yq '.addons.vault.ingress.key' )"
echo "  .Values.addons.vault.ingress.gateway  = $( cat bigbang.yaml | yq '.addons.vault.ingress.gateway' )"
