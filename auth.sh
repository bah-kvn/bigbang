...
#sed -i "s/namespace: default/namespace: vault/g" /tmp/learn-vault-agent/vault-agent-k8s-demo/vault-auth-service-account.yaml
...
kubectl apply --filename /tmp/learn-vault-agent/vault-agent-k8s-demo/vault-auth-service-account.yaml -n vault
...
vault write auth/kubernetes/role/example bound_service_account_names=vault-auth bound_service_account_namespaces=vault policies=myapp-kv-ro ttl=24h
...
EOF
kubectl exec --stdin=true --tty=true devwebapp -- cat /var/run/secrets/kubernetes.io/serviceaccount/token
