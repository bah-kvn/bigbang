...
terraform {
  source = "git::git@github.com:boozallen/terraform-aws-rke2-cluster.git//modules/agent-nodepool"
}
...
inputs = {
  ...
  extra_security_group_ids = concat([dependency.server.outputs.cluster_data.cluster_sg], local.env.cluster.extra_security_groups)
